...
* @notice The token is minted by the same amout of user deposit
...
contract RebaseToken is ERC20, Ownable, AccessControl {
    ...
    uint256 private s_interestRate = 5e10; //  5e10/1e18 = 0.000005% per second, this is the global interest rate of the token - when users mint (or receive tokens via transferral), this is the interest rate they will get.
     ...
     * since the last time the user's balance was updated. it's dynamic and changes over time.
     * it is updated when the user interacts with the protocol.
    ...
    function balanceOf(address _user) public view override returns (uint256) {
        ...
        return (currentPrincipalBalance * _calculateUserAccumulatedInterestSinceLastUpdate(_user)) / PRECISION_FACTOR; // if the PRECISION_FACTOR is too high, this result could overflow
    }
        ...
        returns (uint256 linearInterest)
    {
        ...
        // represents the linear growth over time = 1 + (interest rate * time)  interest rate = s_userInterestRate[_user] / PRECISION_FACTOR
